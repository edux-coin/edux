...
#include "ui_zeduxcontroldialog.h"
...
std::list<std::string> ZEduxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZEduxControlDialog::listMints;

ZEduxControlDialog::ZEduxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZEduxControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZEduxControlDialog::~ZEduxControlDialog()
{
    delete ui;
}

void ZEduxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEduxControlDialog::updateList()
{
...
void ZEduxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEduxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZEdux_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEduxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZEduxControlDialog::GetSelectedMints()
{
...
void ZEduxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
