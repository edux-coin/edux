                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zEDUX ought to be enough for anybody." - Bill Gates, 2017
    ui->zEDUXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzEDUXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzEDUX->setEnabled(false);
        ui->pushButtonMintzEDUX->setToolTip(tr("zEDUX is currently disabled due to maintenance."));

        ui->pushButtonSpendzEDUX->setEnabled(false);
        ui->pushButtonSpendzEDUX->setToolTip(tr("zEDUX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zEDUXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzEDUX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEDUX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zEDUX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zEDUX in ") + 
...
void PrivacyDialog::on_pushButtonSpendzEDUX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEDUX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zEDUX
        sendzEDUX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zEDUX
    sendzEDUX();
}

void PrivacyDialog::on_pushButtonZEduxControl_clicked()
{
    ZEduxControlDialog* zEduxControl = new ZEduxControlDialog(this);
    zEduxControl->setModel(walletModel);
    zEduxControl->exec();
}

void PrivacyDialog::setZEduxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzEduxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzEDUX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Edux Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zEDUXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zEDUX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " EDUX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zEDUX</b>";
    ...
    // use mints from zEdux selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZEduxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZEduxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zedux selector in case it was used
    ZEduxControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zEdux Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Edux, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zEdux Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEDUX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zEDUX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zEDUX "));
    ui->labelzEDUXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzEDUXSyncStatus->setVisible(fShow);
}
